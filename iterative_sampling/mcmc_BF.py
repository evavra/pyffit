...
#Run the inversion based on R-based sampled data iteratively based on best-fitting models.
import os
import time
import emcee
import numba
...
from setup_BF import priors, labels, units, scales, log_prob
...
def inversion():
    ...
    n_process = 10                                    # number of threads to use for parallelization
    ...
    iteration       = 4 #Tracking the number of iteration!! 
    out_dir         = 'real_results_BF4'
    inversion_mode  = 'reload' # 'check_downsampling' to only  make quadtree downsampling plots, 'run' to run inversion, 'reload' to load previous results and prepare output products
    ...
    #Zoom in into the contours
    if inversion_mode == 'reload':
        limits=[[2.6,2.725],[7.35,7.45],[2.075,2.2],[23.1,23.7],[18.9,19.6],[24.2,24.45],[82.7,83.2],[1.68,1.74],[-0.11,-0.09]]
