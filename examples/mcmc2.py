...
from setup import priors, labels, units, scales, log_prob, inversion_mode, out_dir
...
def inversion():
    ...
    n_process = 60                                    # number of threads to use for parallelization
    ...
    # out_dir and inversion_mode are imported from setup.py
    # Set inversion_mode there to: 'check_downsampling', 'run', or 'reload'

    # ---------------------------------- INVERSION ----------------------------------  #
    # If only checking downsampling (plots already made in setup.py), nothing to do here
    if inversion_mode == 'check_downsampling':
        print('Downsampling check complete. See quadtree plots in:', out_dir)
        return
